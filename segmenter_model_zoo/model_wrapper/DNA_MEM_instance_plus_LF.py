...
def SegModule(img=None, model_list=None, prune_border=False, filename=None, index=None, return_prediction=False, two_camera=False):
    ...
    # model 4: dna from bf
    if two_camera:
        dna_bf_pred = model_list[3].apply_on_single_zstack(input_img = img[2,:,:,:], use_tta=False)
    else:
        dna_bf_pred = model_list[3].apply_on_single_zstack(input_img = img[2,:,:,:], use_tta=True)
    dna_bf_bw = dna_bf_pred > dna_bf_cutoff
    imsave('lf_dna.tiff', dna_mask_pred)

    # model 5: mem from bf
    if two_camera:
        mem_bf_pred = model_list[4].apply_on_single_zstack(input_img = img[2,:,:,:],use_tta=False)
    else:
        mem_bf_pred = model_list[4].apply_on_single_zstack(input_img = img[2,:,:,:],use_tta=True)
