...
def SegModule(img=None, model_list=None, filename=None, index=None, return_prediction=False):
    ...
    fill_pred = model_list[0].apply_on_single_zstack(lamin_img, already_normalized=True, cutoff=-1)
    ...
    # model 2: core
    core_pred = model_list[1].apply_on_single_zstack(lamin_img, already_normalized=True, cutoff=-1)
    ...
    # watershed
    seed = label(core_bw)
    final = watershed(1.01-fill_smooth, seed.astype(int), mask=pre_mask, watershed_line=True)   
    ...
    for cid in range(num_cell):
        ...
        for zz in range(tmp_cell.shape[0]):
            tmp_z = tmp_cell[zz,:,:]
            if np.any(tmp_z):
                tmp_cell[zz,:,:] = remove_small_objects(binary_fill_holes(tmp_z), min_size = 100)
