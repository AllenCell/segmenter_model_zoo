...
def SegModule(img=None, model_list=None, prune_border=False, filename=None, index=None, return_prediction=False, two_camera=False):
    ...
    # model 2: dna_seed
    seed_pred = model_list[1].apply_on_single_zstack(dna_img, already_normalized=True, cutoff=-1)
    seed_bw = seed_pred>seed_bw_th

    # model 3: mem from bf
    if two_camera:
        mem_bf_pred = model_list[2].apply_on_single_zstack(input_img = img[2,:,:,:],use_tta=False)
    else:
        mem_bf_pred = model_list[2].apply_on_single_zstack(input_img = img[2,:,:,:],use_tta=True)
    ...
    dna_bf_bw_augmented = remove_small_objects(dna_bf_bw_augmented, min_size=min_seed_size, connectivity=1)
    ...
    for ii in range(num_pred_dna):
        ...
        if np.count_nonzero(np.logical_and(single_extra, seed_bw))<50:
            ...
            dna_mask_bw[single_extra>0]=1 # if this seed is missing, assumes the signle of this dna is problematic
    ...
    for ii in np.unique(cell_seg[cell_seg>0]):
        ...
        if np.count_nonzero(this_one_cell>0)< 70000 or np.count_nonzero(getLargestCC(this_dna, is_label=False))< 10000: # small "cell" or "dna"
    ...
    for cell_idx in range(num_cell):
        ...
        assert np.any(dna_mask_label==(cell_idx+1))  # empty dna should have been removed
    ...
    if len(rois_array)>0:
        ...
        if len(cell_pairs)>0:
            ...
            if exist_double_assignment(cell_pairs):
                ...
                multi_pairs_aux_score = [cell_pairs_aux_score[ii] for ii in multi_pair_index ]
                ...
                while True:
                    ...
                    if len(multi_pairs)==0 or (not exist_double_assignment(multi_pairs)):
                        ...
                        multi_pairs_aux_score = multi_pairs_aux_score[multi_pair_index_new]
